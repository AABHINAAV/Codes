// Problem Link - https://www.codechef.com/MARCH21A/problems/WIREL
//Username : arpit0891
/* By Arpit G. mishra */
#include<bits/stdc++.h>
...
void solve()  {
    ll x1,y1,x2,y2;
        cin>>x1>>y1>>x2>>y2;
    if(z==0) cout<<(-x1-1)<<" "<<(-y1);
    else if(z == 1) cout<<(1000001-x1)<<" "<<(1000000-y1);
	else
{
	int t1 = (1000001-x1)*(1000001-x1) +(1000000-y1)*(1000000-y1);
	int t2 = (-x1-1)*(-x1-1) + (-y1)*(-y1);
	if(t1<t2) 
	{
    	cout<<(1000001-x1)<<" "<<(1000000-y1);
