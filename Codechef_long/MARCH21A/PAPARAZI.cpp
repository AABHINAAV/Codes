// Problem Link - https://www.codechef.com/MARCH21A/problems/PAPARAZI
//Username : arpit0891
/* By Arpit G. mishra */
#include<bits/stdc++.h>
...
struct modular_int{
	...
	template<typename T>
	friend modular_int operator + (const T &l, const modular_int<MOD>& vpec1){
		return (modular_int<MOD>(l) + vpec1);
	}
	template<typename T>
	friend modular_int operator - (const T &l, const modular_int<MOD>& vpec1){
		return (modular_int<MOD>(l) - vpec1);
	...
	friend istream& operator >> (istream& is, modular_int<MOD>& vpec1){
		int64_t val;
		is >> val;
		vpec1.x = (val%MOD);
		if (vpec1.x < 0) vpec1.x += MOD;
		return is;
	}
	friend ostream& operator << (ostream& os, const modular_int<MOD>& vpec1) {return os<<vpec1.x;}
...
void solve()  {
        ll n;
        vector<pair<ll,ll>> vpec1;
        vector<pair<ll,ll>> vpec2;
        cin>>n;
        ...
        vpec2.pb(vpec1[0]);
        vpec2.pb(vpec1[1]);
        ll out=1, space2=vpec2.size();
        loop(i,2,n-1){
            while(vpec2.size()>=2) {
                ld s1 = ((ld)vpec2[space2-1].second - (ld)vpec2[space2-2].second)/((ld)vpec2[space2-1].first - (ld)vpec2[space2-2].first);
                ld s2 = ((ld)vpec1[i].second - (ld)vpec2[space2-1].second)/((ld)vpec1[i].first - (ld)vpec2[space2-1].first);
                if(s1 <= s2) {
                    vpec2.pop_back();
                    space2--;
                } 
            ...
            vpec2.pb(vpec1[i]);
            space2=space2+1;
            out = max(out, vpec2[space2-1].first - vpec2[space2-2].first);
        }
        cout<<out;
    }
int main(int argc, char const *argv[]) {
	...
	#ifndef ONLINE_JUDGE 
	  clock_t end = clock();
	  cout<<"\n\nExecuted In: "<<ld(end - begin) / CLOCKS_PER_SEC*1000<<" ms";
	#endif 
	return 0;
}
