// Problem Link - https://www.codechef.com/MARCH21A/problems/MAXTOPO
//Username : arpit0891
/* By Arpit G. mishra */
#include<bits/stdc++.h>
...
ll fun1(ll i, ll xx)
{
 ...
 for (auto xxx : G[i])
 {
  if (xxx == xx)
   continue;
  ll o = (fun1(xxx, i) * dp[xxx]) % mod;
  mul =  (o * mul) % mod;
  dp[i] += dp[xxx];
 }
 return mul;
}
ll fun2(ll x, ll xx, ll n)
{
 ll siz = 1;
 ll counter = 1;
 for (auto a : G[x])
...
F(i,n-1)
 ...
 {
  ll m = fun1(a, c[0]);
  mul = ((( m * dp[a]) % mod) * mul) % mod;
 }
 ll f = (xxxx[n - 1] * inv(mul)) % mod;
 ...
 if (c.size() == 1)
 {
  if (k == 1) {
   ...
   cout << res << " " << (((xx * inv(n - xx)) % mod) *f) % mod;
   return;
  }
...
int main(int argc, char const *argv[]) {
	...
	cin>>t;
	xxxx[0] = 1;
    for (ll i = 1; i < 500001; i++) xxxx[i] = (xxxx[i - 1] * i) % mod;
	while (t--){
		solve();
		cout<<"\n";
		c.clear();
