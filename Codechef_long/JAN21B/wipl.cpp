...
ll func(ll i,ll j,ll n,ll k,vi &v)
{
    if(j>=k&&(pr[i]-j>=k))
   return 0;
    else if(i>=n)
   ...
   return dp[i][j];
    return dp[i][j]=1+min(func(i+1,min(j+v[i],pr[i]-j),n,k,v),func(i+1,min(pr[i]-j+v[i],j),n,k,v));
}
ll memo(vi &v,ll n,ll k)
{   F(i,n/2)
   swap(v[i],v[n-i-1]);
   loop(i,0,n+10)
  {
    ...
    loop(i,1,n) pr[i]=pr[i-1]+v[i-1];
    ll output=func(0,0,n,k,v);
    if (output>mod) return -1;
    else return output;
}
ll knpsk(ll i,ll c,ll d,vi &a,ll k){
ll x=inf,y=inf;
if (c>4000) c=4000;
else if (d>4000) d=4000;
if((c>=k) &&(d>=k)) return dp[c][d];
else if(i>=a.size())  return 1e12;
else if(dp[c][d]!=-1) return dp[c][d];
if(c < k) x = 1 + knpsk(i+1,c+a[i],d,a,k);
if(d < k) y = 1 + knpsk(i+1,c,d+a[i],a,k);
dp[c][d]=min(x,y);return dp[c][d];
}
void solve(){
    ll n,k,out,xc=0;
    cin>>n>>k;
