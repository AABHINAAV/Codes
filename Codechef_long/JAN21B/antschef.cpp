		...
		F(i,n){
			...
			F(j,d){
				...
				if (mapping[abs(ant[i][j])] > 1)
				{
					...
					if (ant[i][j]>0) c=lower_bound(ant[i].begin(),ant[i].end(),-1*ant[i][j])-ant[i].begin();
					else c=ant[i].size()-(upper_bound(ant[i].begin(),ant[i].end(),-1*ant[i][j])-ant[i].begin());
