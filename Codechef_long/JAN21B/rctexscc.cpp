// Problem Link - https://www.codechef.com/JAN21B/problems/RCTEXSCC
//Username : arpit0891
/* By Arpit G. mishra */
#include<bits/stdc++.h>
...
void solve(){
   ...
   y = power(y, mod - 2);
      ...
      dp[0][1] = (2 * k * (k - 1)) % mod;
      for(int i = 1; i < n; i++) {
        dp[i][0] += ((k - 1) * (dp[i - 1][0] + (k * power(k, 2 * (i - 1))) % mod) % mod) % mod; dp[i][0] %= mod;
        dp[i][0] += dp[i - 1][0]; dp[i][0] %= mod;
        dp[i][0] += (2LL * dp[i - 1][1]) % mod; dp[i][0] %= mod;
        dp[i][0] += ((k - 2) * (dp[i - 1][1] + (((k * (k - 1)) % mod) * power(k, 2 * (i - 1))) % mod) % mod) % mod; 
        dp[i][0] %= mod;
        dp[i][1] += ((k - 1) * (dp[i - 1][0] + (k * power(k, 2 * (i - 1))) % mod) % mod) % mod; dp[i][1] %= mod;
        dp[i][1] += ((k - 1) * (dp[i - 1][0] + (k * power(k, 2 * (i - 1))) % mod) % mod) % mod; dp[i][1] %= mod;
