// Problem Link - https://www.codechef.com/FEB21A/problems/ATWNT
//Username : arpit0891
/* By Arpit G. mishra */
#include<bits/stdc++.h>
...
#define printer();          F(i,q) cout<<ans[i]<<"\n";
#define helper();       void dfshelper(int u){if(g[u].size()==0){node[u]=1;}for(auto i:g[u]){dfshelper(i);node[u]+=node[i];}}
template<typename T>
...
#define merger();     void merge(int x,int y){if(queries[x].size()<queries[y].size())swap(queries[x],queries[y]);for(auto i:queries[y])queries[x].pb(i);}
...
unordered_map<int,vector<int>*>  height[MXN];
    ...
    if(g[u].size()==1){
        ...
        for (auto &i:queries[u]) {
            if (i.ff % sz != 0) {
                ...
                if(height[lvl+1].count(i.ff/sz)==0){

                    height[lvl+1][i.ff/sz]=new vector<int>();
                }

                height[lvl+1][i.ff/sz]->pb(i.ss);
            }
        }
    }
    for(auto i:g[u]){
