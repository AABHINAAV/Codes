// Problem Link - https://www.codechef.com/FEB21A/problems/MULGAME
//Username : arpit0891
/* By Arpit G. mishra */
#include<bits/stdc++.h>
...
#define max    int mi=-1;i=0;while (i<=m){mi=max(b[i],mi);i++;}
#define wtf     for(auto x:mapping){int k=x.ff.ff,p=x.ff.ss,l=x.ss;b[p+k]-=l,b[p+2*k]+=l;int c=p+2*k;while(c<=m-p){c+=p;b[c]-=l,b[c+k]+=l;c+=k;}}
